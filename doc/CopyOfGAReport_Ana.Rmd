...
1. Based on the number of page views, we can group the table gadata, and find the ones with more views sorting the column (showing the first 10):

```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
visitspmonth <- group_by(gadata, pagePath) %>% summarise(pageviews = sum(pageviews))
visitspmonth2 <- visitspmonth[order(-visitspmonth$pageviews),]
t2 <- gvisTable(visitspmonth2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(visitspmonth2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')
...
```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
nocpc = gadata[gadata$Medium != "cpc",]
visitspmonth_cpc <- group_by(nocpc, pagePath) %>% summarise(pageviews = sum(pageviews))
visitspmonth_cpc2 <- visitspmonth_cpc[order(-visitspmonth_cpc$pageviews),]
t3 <- gvisTable(visitspmonth_cpc2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(visitspmonth_cpc2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t3, 'chart')

```


2. To find the pages with more exits, the process is the same as before, although filtering by exits:

```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
exitspmonth <- group_by(gadata, pagePath) %>% summarise(Exits = sum(exits))
exitspmonth2 <- exitspmonth[order(-exitspmonth$Exits),]
t2 <- gvisTable(exitspmonth2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(exitspmonth2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')

```

According to the description, Exits are the number of times that page was the last one opened by a user. Some of the reasons may come from the look and feel of the website, or by how difficult it is to navigate through it. Thus, improving the design and the usability of the website might help.

3. This case, in terms of procedure, is the same as before, although filtering by time on page:

```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
timeonpage <- group_by(gadata, pagePath) %>% summarise(timeonpage = sum(timeOnPage))
timeonpage2 <- timeonpage[order(-timeonpage$timeonpage),]
t2 <- gvisTable(timeonpage2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(exitspmonth2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')

```

when the exit rate matches a low time on the website, then it might be advisable to write more on the topic. However, if the reason for exiting the page is the difficulty to navigate, then the solution will be related to the structure of the website, not on the content. 

4. For this case, we create a combination of the sources and medium and check both the number of new visitors and their exists. The following are the pages more visited by new visitors:

```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
newvis = gadata[gadata$userType == "New Visitor",]
combomonth <- group_by(newvis, combo = paste(newvis$source, newvis$Medium, sep = " ")) %>% summarise(Users = sum(users), Exits = sum(exits))
combomonth2 <- combomonth[order(-combomonth$Users),]
t2 <- gvisTable(combomonth2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(combomonth2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')

```


We then can check, for those combinations, what are the exit rates for Returning Visitors (much lower): 

```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
retvis = subset(gadata, userType == 'Returning Visitor')
combomonth_rv <- group_by(retvis, combo = paste(retvis$source, retvis$Medium, sep = " ")) %>% summarise(Users = sum(users), Exits = sum(exits))
combomonth_rv2 = subset(combomonth_rv, is.element(combo, combomonth2$combo[1:10]))
t2 <- gvisTable(combomonth_rv2[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(combomonth_rv2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')

```

5. The procedure here is the same as before, but instead of combining source and medium, we only want to check when medium = Referral


```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
referrals = gadata[gadata$Medium == "referral",]
referrals2 <- group_by(referrals, source) %>% summarise(pageviews = sum(pageviews))
referrals3 <- referrals2[order(-referrals2$pageviews),]
t2 <- gvisTable(referrals3[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(referrals2) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')
...
```{r, echo=TRUE, message=FALSE, prompt=FALSE, results='asis'}
referralscpc = gadata[gadata$Medium == "cpc",]
referralscpc2 <- group_by(referralscpc, source) %>% summarise(pageviews = sum(pageviews))
referralscpc3 <- referralscpc2[order(-referralscpc2$pageviews),]
t2 <- gvisTable(referralscpc3[1:10,], options = list(showRowNumber = FALSE, width = 800, height = min(200,27*(nrow(referralscpc3) + 1)), allowHTML = TRUE, page = 'disable'))
print(t2, 'chart')
...
THe first thing I notice is that for cpc, the only source is google, whereas for referrals, there are several sources, and google is not among the first 10. 
